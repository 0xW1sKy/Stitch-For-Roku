sub init()
    ...
    m.getStuffVideo = createObject("roSGNode", "GetSwitchStuffVideo")
...
sub onGetFocus()
    ? "Home Scene > Channel Page > onGetFocus"
    if m.top.visible
...
sub onGetVideoUrl()
    ? "Home Scene > Channel Page > onGetvideoUrl"
...
sub onVideoItemSelect()
    ? "Home Scene > Channel Page > onVideoItemSelect"
    if m.pastBroadcastsList.hasFocus()
...
sub onGetVideos()
    ? "Home Scene > Channel Page > onGetVideos"
    newList = false
...
sub onGetUserInfo()
    ...
    m.top.backgroundImageUri = m.streamItem.content.getChild(0).getChild(0).HDPosterUrl
    ? "Home Scene > Channel Page > onGetUserInfo > backgroundImageUri"; m.top.backgroundImageUri
...
sub onSelectedStreamerChange()
    ? "Home Scene > Channel Page > onSelectedStreamerChange"
    content = createObject("roSGNode", "ContentNode")
    row = createObject("RoSGNode", "ContentNode")
    rowItem = createObject("RoSGNode", "ContentNode")
...
sub onKeyEvent(key, press) as boolean
    ' ? "Home Scene > Channel Page > onKeyEvent"
    handled = false
    ...
    if press
        if key = "up" and m.pastBroadcastsList.hasFocus()
            ...
            ' ? "Home Scene > Channel Page > onKeyEvent > Back"
            m.pastBroadcastsList.content = invalid
        end if
    end if

...
function handleDownKey(key, pres)

end function

function onKeyEvent(key, press) as boolean
    handled = false
    if press and m.total_channels > 0'and m.currentlyFocused <= m.total_channels
        ...
        if handled and m.total_channels > 0 'and m.currentlyFocused <= m.total_channels
        ...
        if key = "down" and m.total_channels > 0'and m.currentlyFocused <= m.total_channels
            ' if m.children[m.currentlyFocused] <> invalid
            m.children[m.currentlyFocused].getChild(0).opacity = 1
            ' end if
            'm.children[m.currentlyFocused].getChild(1).visible = true
            'm.indicator.getChild(0).text = m.children[m.currentlyFocused].getChild(1).text
